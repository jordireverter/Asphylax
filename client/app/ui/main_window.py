...
from PyQt6.QtGui import QBrush, QColor, QFont, QIcon, QLinearGradient, QPainter, QPainterPath, QPalette, QPen, QRadialGradient
...
class AsphylaxUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Asphylax")
        self.setGeometry(200, 200, 1200, 750)
        self.setWindowIcon(QIcon(str(GUI_DIR / "icons" / "logo.ico")))
        ...
        logo_label = QLabel()
        logo_size = 72
        pixmap = QIcon(str(GUI_DIR / "icons" / "logo.svg")).pixmap(
        ...
        name_label.setFont(QFont("Segoe UI", 26, QFont.Weight.Bold))
        name_label.setStyleSheet("color: #d84a4a; margin-left: 14px;")
    ...
    def format_timestamp(self, raw_date: str) -> str:
        ...
        try:
            ...
            # Si el timestamp porta zona horària (+00:00), el convertim a hora local
    ...
    def create_scan_tab(self):
        ...
        title = QLabel("Escaneig de fitxers")
    ...
    def create_settings_tab(self):
        ...
        self.config_excluded_paths = self.create_config_text_edit("Una ruta per línia", 90)

        self.config_excluded_extensions = self.create_config_text_edit("Una extensió per línia, ex: .tmp", 70)
        ...
        self.config_quick_extensions = self.create_config_text_edit("Una extensió per línia, ex: .exe", 90)
        self.config_quick_excluded_dirs = self.create_config_text_edit("Una carpeta per lí­nia, ex: node_modules", 90)

        general_form = QFormLayout()
        general_form.addRow("Tema visual", self.config_theme)
        general_form.addRow("Mida màxima YARA", self.config_yara_size)
        ...
        engines_form.addRow("Longitud mínima Base64", self.config_base64_length)
        ...
        quick_form.addRow("Mida màxima", self.config_quick_max_size)
        ...
        content_layout.addWidget(self.create_config_group("Escaneig ràpid", quick_form))

        quarantine_form = QFormLayout()
        quarantine_form.addRow(self.config_auto_quarantine_enabled)
        quarantine_form.addRow("Nivell mínim", self.config_auto_quarantine_level)
    ...
    def on_monitor_event_received(self, path: str, action: str):
        ...
        if response.get("status") != "ok":
            status = "error"
            result_text = response.get("message", "Error de connexió amb l'agent.")
        else:
            data = response.get("data") or {}
            files = data.get("files", [])
    ...
    def delete_selected_quarantine(self):
        ...
        confirm = QMessageBox.question(
            self, "Confirmar eliminació", "Segur que vols eliminar definitivament aquest fitxer de quarantena?",
